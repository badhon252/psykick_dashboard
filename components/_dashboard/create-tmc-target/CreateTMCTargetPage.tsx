...
export default function CreateTMCTargetPage() {
  ...
  const handleSelectControlImage = (id: string) => {
    if (selectedControlImages.includes(id)) {
      setSelectedControlImages(
        selectedControlImages.filter((imgId) => imgId !== id)
      );
    } else {
  ...
  console.log(data?.data.subCategories.subCategories)

  const visibleTargetImages = showMoreTargets
    ? targetImages
    : targetImages.slice(0, 15);
  const visibleControlImages = showMoreControls
    ? controlImages
    : controlImages.slice(0, 15);
      ...
      <main className="flex-1 p-6 space-y-6">
        <Card className="bg-[#170A2C]/50 border-0">
          <CardContent className="space-y-6">
                ...
                {data?.data.map((image) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedTargetImage === image._id && "border-[#8F37FF]"
                    )}
                    onClick={() => handleSelectTargetImage(image._id)}
                  >
                    <Image
                      src={image.targetImage || "/placeholder.svg"}
                      alt={image.targetImage}
                      width={200}
                      height={150}
                      className="w-full h-32 object-cover"
                    />
                    {selectedTargetImage === image._id && (
            ...
            <div className="space-y-4">
              <h3 className="text-white text-lg">
                Select Control Images(any 5):
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {data?.data.controlImages?.map((image) => (
                  <div
                    key={image}
                    className={cn(
                      "relative rounded-md overflow-hidden cursor-pointer border-2 border-transparent",
                      selectedControlImages.includes(image._id) &&
                        "border-[#8F37FF]"
                    )}
                    onClick={() => handleSelectControlImage(image._id)}
                  >
                    <Image
        ...
        {selectedTargetImage && (
          <div className="mt-8">
            ...
            <div className="border-4 border-red-500 rounded-lg overflow-hidden max-w-2xl mx-auto">
              <Image
                src={
                  targetImages.find((img) => img.id === selectedTargetImage)
                    ?.src || "/placeholder.svg"
                }
                alt="Selected target"
                width={600}
